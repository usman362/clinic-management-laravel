<div class="d-flex justify-content-center">
    ...
    @if ($row->status !== $cancel && $row->status !== $checkOut)
        ...
        <a href="javascript:void(0)" data-id="{{ $row->id }}"
            ...
            data-bs-original-title="Cancel Appointment" data-bs-custom-class="tooltip-dark" data-bs-placement="bottom"
            title="{{ __('messages.appointment.cancel_appointment') }}">
            <i class="fas fa-calendar-times"></i>
        </a>
    @endif
    <a href="{{ route('patients.appointment.detail', $row->id) }}" title="{{ __('messages.common.show') }}"
