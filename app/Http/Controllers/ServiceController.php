...
class ServiceController extends AppBaseController
{
    ...
    public function getService(Request $request)
    {
        ...
        $message = $service->name . ' ' . ' with ' . ($doctor->user->first_name . ' ' . $doctor->user->last_name) . ' (' . $service->duration . ' min)';
        return $this->sendResponse(['service' => $service, 'custom_message' => $message], __('messages.flash.retrieve'));
    }

    public function getCharge(Request $request): JsonResponse
    {
        $chargeId = $request->chargeId;
        $charge = Service::with('serviceDoctors')->find($chargeId);
        if ($charge) {
            $serviceDoctor = DB::table('service_doctor')->where('service_id', $charge->id)->pluck('doctor_id');
            $doctors = Doctor::with('user')->whereIn('id', $serviceDoctor)->get();
        } else {
            $doctors = null;
        }

        return $this->sendResponse(['charge' => $charge, 'doctors' => $doctors], __('messages.flash.retrieve'));
    }
